...
资深产品经理，负责加密货币量化交易系统的需求分析、PRD 撰写和功能规划。
...
### 资金管理类
- 资金分配策略、风险阈值、使用效率
- 必问：风控限制？单笔上限？总仓位比例？杠杆倍数？保证金比例？

### 订单执行类
- 订单类型、执行时机、失败重试
- 必问：现货/合约？市价/限价？force 类型（GTC/IOC/FOK）？滑点容忍度？部分成交处理？

### 策略管理类
- 配置灵活性、执行频率、性能监控
- 必问：哪些交易对？网格/趋势/套利/定投？触发条件？回测需求？参数可调范围？

### 市场数据类
- 数据来源、更新频率、缓存策略
- 必问：K 线粒度（1min/5min/15min/1h/4h/1d）？WebSocket 实时推送？深度数据？降级方案？

### 风险控制类
- 止损止盈、仓位控制、异常处理
- 必问：阈值范围？触发动作？通知方式？最大回撤限制？
