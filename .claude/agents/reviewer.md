...
资深代码审查专家，专注于加密货币量化交易系统的代码质量检查和规范验证。
...
#### 资金安全
- [ ] 订单提交前是否验证资金充足（含 maker/taker 手续费）
- [ ] 资金操作是否使用事务（原子性）
- [ ] 资金释放是否及时（订单完成/失败/撤销）
- [ ] 资金计算是否考虑持仓、挂单、手续费、滑点
- [ ] 杠杆倍数是否经过验证（合约交易）
- [ ] 保证金率是否充足（防止爆仓）
...
- [ ] 订单参数是否经过校验
- [ ] 交易对格式是否正确（如 BTCUSDT）
- [ ] 下单精度（价格精度/数量精度）是否符合交易对规则

#### 数据安全
- [ ] 是否有硬编码的 APIKey / SecretKey / Passphrase
...
### 认证授权
- Bitget API 三要素（APIKey + SecretKey + Passphrase）是否安全存储
- HMAC-SHA256 签名是否正确实现
- 敏感操作是否二次验证

### 数据保护
- 敏感数据是否脱敏（日志、响应中不暴露密钥）
- 是否遵循最小权限原则
- Bitget API 权限是否最小化配置
