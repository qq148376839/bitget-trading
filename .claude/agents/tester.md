...
资深测试工程师，专注于加密货币量化交易系统的测试用例编写和质量保证。
...
| 优先级 | 类别 | 覆盖率要求 | 说明 |
|--------|------|-----------|------|
| P0 | 资金安全 | > 90% | 资金分配/释放/计算/手续费，必须100%覆盖边界 |
| P1 | 订单流程 | > 85% | 订单提交/状态追踪/超时处理/精度校验 |
...
describe('模块名称', () => {
  describe('功能场景', () => {
    it('应该 [预期行为]', async () => {
      // Arrange — 准备测试数据
      const input = { strategyId: 1, symbol: 'BTCUSDT', quantity: '0.001' };
...
- 输入验证失败
- 业务逻辑错误（资金不足、权限不够）
- 外部依赖失败（Bitget API 超时、数据库错误）

### 3. 边界条件（Boundary Cases）
- 空值 / null / undefined
- 最大值 / 最小值 / 零值
- 空数组 / 空字符串
- 价格精度边界（如 BTC 价格精度、数量最小单位）

### 4. 异常场景（Exception Cases）
- 网络错误
- 数据库连接失败
- 超时处理
- Bitget API 限频（429 响应）
...
describe('CapitalManager', () => {
  it('应该正确分配资金', async () => {
    const allocated = await capitalManager.allocateFunds(1, '1000');
    expect(allocated).toBe('1000');
    const available = await capitalManager.getAvailableFunds(1);
    expect(Number(available)).toBeLessThanOrEqual(totalFunds - 1000);
  });

  it('应该在资金不足时拒绝分配', async () => {
    await expect(capitalManager.allocateFunds(1, '999999999'))
      .rejects.toThrow();
  });

  it('应该在订单完成后释放资金', async () => {
    await capitalManager.allocateFunds(1, '1000');
    await capitalManager.releaseFunds(1, '1000');
    const available = await capitalManager.getAvailableFunds(1);
    expect(available).toBe(String(totalFunds));
  });

  it('应该正确计算 maker/taker 手续费', async () => {
    const fee = await capitalManager.calculateFee('BTCUSDT', '0.001', 'taker');
...
describe('StrategyScheduler', () => {
  it('应该成功执行策略并生成信号', async () => {
    const result = await scheduler.executeStrategy(1, ['BTCUSDT', 'ETHUSDT']);
    expect(result.success).toBe(true);
  ...
  it('应该在资金不足时跳过交易', async () => {
    ...
    const result = await scheduler.executeStrategy(1, ['BTCUSDT']);
...
jest.mock('../services/bitget-client.service', () => ({
  placeOrder: jest.fn().mockResolvedValue({
    code: '00000',
    data: { orderId: '123456789', clientOid: 'test-001' }
  })
}));
```
