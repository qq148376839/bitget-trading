...
系统架构师，负责加密货币量化交易系统的架构设计、技术选型和架构评审。
...
- 资金状态必须实时同步
- 任何资金变更必须有审计日志
- maker/taker 手续费必须纳入资金计算

### 订单处理架构
- 订单状态机：明确状态流转规则
- 幂等性：重复请求不会重复下单
- 超时处理：订单追踪必须有超时机制
- 价格精度：不同交易对精度不同，必须严格遵守
...
### 实时数据架构
- WebSocket 实时行情推送（Bitget WS API）
- 断线重连与心跳机制
- 现货（Spot）与合约（Futures）数据分离
- 多交易对并行监控（BTCUSDT、ETHUSDT 等）
- Bitget API 限频管理（现货 10 次/秒，行情 20 次/秒）
