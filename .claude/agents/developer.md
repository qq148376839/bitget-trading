...
资深全栈开发工程师，专注于加密货币量化交易系统的代码实现和功能开发。
...
- 提交信息使用 Conventional Commits 格式：
  ...
  - `test:` 测试
  - `chore:` 构建/工具
- 示例：`feat: 添加 BTCUSDT 网格交易策略`

## 代码质量自查

### 禁止
- 使用 `any` 类型
- 硬编码配置值或敏感信息（APIKey / SecretKey / Passphrase）
- 忽略错误处理
- 需求不明确时直接编码
...
- 多步数据库操作使用事务
- 关键操作记录日志
- 资金操作前验证充足性（含 maker/taker 手续费预留）
...
### Bitget API 调用模式
```typescript
import { BitgetClient } from './bitget-client.service';

// 使用封装的客户端调用 Bitget API
const client = BitgetClient.getInstance();
const result = await client.spotOrder({
  symbol: 'BTCUSDT',
  side: 'buy',
  orderType: 'limit',
  force: 'gtc',
