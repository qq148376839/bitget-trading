...
#### Bitget API 密钥管理
- [ ] APIKey 是否存储在环境变量中
- [ ] SecretKey 是否存储在环境变量中
- [ ] Passphrase 是否存储在环境变量中
- [ ] 是否有硬编码的密钥或 Token
- [ ] 密钥是否通过安全通道传输
- [ ] API 权限是否最小化（只开启读取+交易，禁止提现）

#### HMAC-SHA256 签名安全
- [ ] 签名算法是否正确实现
- [ ] 时间戳是否防重放攻击（合理窗口期）
- [ ] 签名的 prehash 字符串构造是否正确
...
#### 日志脱敏
- [ ] 日志是否包含敏感信息（SecretKey、Passphrase、APIKey）
- [ ] 错误响应是否泄露内部实现细节
- [ ] 调试日志在生产环境是否关闭

#### 输入验证
- [ ] 所有外部输入是否验证类型和范围
- [ ] 交易对名称是否验证格式（如 BTCUSDT）
- [ ] 价格和数量是否验证精度和范围
...
#### 资金操作验证
- [ ] 资金变更是否使用数据库事务
- [ ] 资金计算是否使用字符串/大数运算（避免浮点误差）
- [ ] 资金操作是否有审计日志
- [ ] 是否有资金异常告警机制
- [ ] maker/taker 手续费是否正确计算
